# Terms of Service - نشأة

**Last Updated:** June 20, 2026

These Terms of Service ("Terms") govern your use of the نشأة mobile application ("App") operated by نشأة ("we," "our," or "us").

By using the App, you agree to be bound by these Terms. If you do not agree, do not use the App.

---

## 1. Account Access

- Accounts are created by the nursery administration for authorized parents and staff members
- You are responsible for maintaining the confidentiality of your login credentials
- You must notify the nursery administration immediately of any unauthorized use of your account
- One account per person; sharing accounts is not permitted

---

## 2. Acceptable Use

You agree to use the App only for its intended purposes:

- Communicating with nursery staff about your child's care
- Viewing attendance records and daily reports
- Managing pickup requests
- Viewing invoices and financial information

You agree NOT to:

- Share confidential information about other children or families
- Use the messaging system for commercial or promotional purposes
- Attempt to access data belonging to other users
- Reverse engineer or modify the App
- Use the App in any way that violates applicable laws

---

## 3. Content

- Messages and photos shared through the App remain the property of the sender
- The nursery administration reserves the right to moderate and remove inappropriate content
- You grant us a limited license to store and transmit content you share through the App for the purpose of providing the service

---

## 4. Push Notifications

- The App may send push notifications for important events (pickup alerts, messages, daily reports)
- You can disable notifications at any time through your device settings
- Critical safety notifications (pickup verification) may require notification access for full functionality

---

## 5. Biometric Authentication

- The App offers Face ID / Touch ID for convenient authentication
- Biometric data is processed locally on your device
- We never receive, store, or transmit your biometric data
- Biometric authentication is optional; you can use password authentication instead

---

## 6. Availability

- We strive to maintain App availability but do not guarantee uninterrupted service
- The App includes offline functionality for viewing cached data when internet is unavailable
- Scheduled maintenance will be communicated in advance when possible

---

## 7. Privacy

Your use of the App is also governed by our Privacy Policy, available at https://portal.naashahco.com/privacy.

---

## 8. Limitation of Liability

- The App is provided "as is" without warranties of any kind
- We are not liable for any indirect, incidental, or consequential damages
- Our total liability is limited to the fees paid for the service (if any)
- We are not responsible for decisions made based on information displayed in the App

---

## 9. Termination

- The nursery administration may deactivate your account at any time
- Upon termination, your access to the App will be revoked
- Data retention after termination follows our Privacy Policy

---

## 10. Changes to Terms

We may modify these Terms at any time. Continued use of the App after changes constitutes acceptance of the new Terms.

---

## 11. Governing Law

These Terms are governed by the laws of the Kingdom of Saudi Arabia.

---

## 12. Contact

For questions about these Terms, contact:

- **Email:** support@naashahco.com
- **Website:** https://portal.naashahco.com
